...
p = zeros(96, 2);  % position of antennas on BS, three circle with r = 0.05, 0.06 and 0.07
for i = 1 : 16
...
for i = 33 : 48
    ...
    p(i,:) = 0.25 * [cos(alpha_i) sin(alpha_i)]; 
end
for i = 49 : 64
    alpha_i = (i-48) / 16 * 2 * pi; % angle of each antenna
    p(i,:) = 0.35 * [cos(alpha_i) sin(alpha_i)]; 
end
for i = 65 : 80
    alpha_i = (i-64) / 16 * 2 * pi; % angle of each antenna
    p(i,:) = 0.45 * [cos(alpha_i) sin(alpha_i)]; 
end
for i = 81 : 96
    alpha_i = (i-80) / 16 * 2 * pi; % angle of each antenna
    p(i,:) = 0.55 * [cos(alpha_i) sin(alpha_i)]; 
end
...
for i = 1 : 96
    ...
    w(i) = w_ampl(i) * exp(-1i * 2*pi * dot(p(i,:), r0));
...
for j = 1 : N
    for i = 1 : 96
        b(j) = b(j) + w(i) * exp(1i * 2*pi * dot(p(i,:), r(j,:)));
    end
